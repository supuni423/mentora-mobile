...
Mentora is a Flutter student-side client (Dart SDK ^3.13.1) for the Mentora tutoring platform, consuming an existing Node/Express + PostgreSQL backend at `C:\Users\User\Desktop\Mentora\backend` (see that repo's own `CLAUDE.md` for backend internals and known quirks). Current scope is the "core loop" (auth, course browsing, student profile, enrollment/booking requests) plus an AI-powered course recommendations tab. Tutor/admin features, the communities/feed module, and Socket.io real-time updates are not implemented here.
...
- **`lib/theme/app_theme.dart`** — `AppColors` (mint/peach/cream palette) and `AppTheme.light()`, applied once via `MaterialApp.theme` in `main.dart`. Component themes (cards, buttons, inputs, nav bar) are set globally here so individual screens don't hardcode colors — only genuinely semantic colors (star ratings, delete/error actions) are hardcoded per-screen.
- **Course / Profile / Enrollment / Recommendation** each follow the same `models/*.dart` (defensive `fromJson`, matched against real live backend responses — see the field-shape notes below) → `services/*_service.dart` → `providers/*_provider.dart` → `screens/*/` pattern.
- **Recommendations** (`lib/screens/recommendations/recommendation_screen.dart`, the "For You" tab) posts a preference form to `POST /api/recommendations` (public, no auth) and renders the returned `matchScore` + Gemini-written `aiInsight` per course. Backend contract lives in `C:\Users\User\Desktop\Mentora\backend\src\services\recommendationService.js` — it's a real rule-based-score + Gemini-explanation feature, not a stub; `mode` must be exactly `"Online"`/`"Physical"`/`"Both"` and the response mixes snake_case and camelCase keys (`tutor_id` vs `matchScore`) that the model copies as-is rather than normalizing.
